...
function MeLogo() {
  return (
    <div className="flex items-center gap-2 select-none" style={{ lineHeight: 1 }}>
      <img src="https://cdn.me.com.br/logos/me_primary.png" alt="ME" style={{ height: '26px', width: 'auto' }} />
      <span style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif", fontSize: '20px', fontWeight: 700, color: '#1a1a2e', letterSpacing: '-0.4px' }}>
        ZenDesk
      </span>
      <span style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif", fontSize: '20px', fontWeight: 400, color: '#4a5568', letterSpacing: '-0.4px' }}>
        Analytics
      </span>
    </div>
  );
...
export function Header() {
  ...
  function handleStop() { stopSearch(); setLoading(false); }
  ...
  return (
    <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between gap-4 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        {error && (
          <span className="text-xs text-rose-600 bg-rose-50 border border-rose-200 px-3 py-1 rounded-full">{error}</span>
        )}
        {tickets.length > 0 && !isLoading && (
          <span className="text-xs text-gray-400">{formatNumber(tickets.length)} tickets</span>
        )}
        {isLoading && (
          <>
