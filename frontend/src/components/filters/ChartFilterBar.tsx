...
export function ChartFilterBar() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-blue-50 border border-blue-100 rounded-xl">
      <span className="text-xs text-blue-600 font-semibold uppercase tracking-wide mr-1">
        Filtros ativos:
      </span>
      ...
      {chartFilter.cliente && (
        <Chip
          label="Cliente"
          value={chartFilter.cliente}
          onRemove={() => clearChartFilterKey('cliente')}
        />
      )}
      {chartFilter.tipo && (
        <Chip
          label="Tipo"
          value={chartFilter.tipo}
          onRemove={() => clearChartFilterKey('tipo')}
        />
      )}
      {chartFilter.subtipo && (
        <Chip
          label="Subtipo"
          value={chartFilter.subtipo}
          onRemove={() => clearChartFilterKey('subtipo')}
        />
      )}
      {chartFilter.heatmap && (
        <Chip
          label="Heatmap"
          value={`${chartFilter.heatmap.subtipo} · ${chartFilter.heatmap.data}`}
