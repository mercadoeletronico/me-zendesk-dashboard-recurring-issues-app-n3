...
export function computeKpis(tickets: Ticket[]): KpiData {
  ...
  // Subtipo mais frequente (por contagem de tickets)
  ...
  // Maior Recorrência GERAL: distinct dates per subtipo across ALL clients combined
  // Excludes tickets without subtipo ('-')
  ...
  // Maior Recorrência POR CLIENTE: max t.recorrencia (per cliente+subtipo pair, set by backend)
  // Excludes tickets without subtipo. Collects ALL unique (cliente, subtipo) pairs tied at max.
  ...
  if (validTickets.length > 0) {
    ...
    const seenCliEntries = new Set<string>();
    const cliEntries: Array<{ cliente: string; subtipo: string }> = [];
    validTickets
      ...
      .forEach((t) => {
        const key = `${t.cliente}||${t.subtipo}`;
        if (!seenCliEntries.has(key)) {
          seenCliEntries.add(key);
          cliEntries.push({ cliente: t.cliente, subtipo: t.subtipo });
        }
      });
    maiorRecorrenciaCliente = cliEntries.length > 0
      ? { recorrencia: maxRec, entries: cliEntries }
      : null;
  }
